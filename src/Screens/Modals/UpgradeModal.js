...
const UpgradeModal = ({navigation, route}) => {
  ...
  const {purchasePremium, premiumError, offerings, productIdentifier} =
    usePremium();
  const currentProductIdentifier = useRef(productIdentifier);
  ...
  useEffect(() => {
    ...
    if (currentProductIdentifier.current !== productIdentifier) {
      navigation.goBack();
    }
  }, [productIdentifier]);
      ...
      return (
        <View style={styles.contain}>
          ...
          <Text style={styles.text}>
            ...
            <Text style={styles.small}>You can extend 3 min audio dates</Text>
          </Text>
        </View>
      );
    }
      ...
      <Animated.View style={[styles.wrapper, {bottom: slideBottom}]}>
        <View style={styles.box}>
          ...
          <View style={styles.body}>
            <Text style={styles.can}>Upgrade your plan</Text>
            <Carousel
              ref={carouselRef}
              data={data}
          ...
          <View style={styles.footer}>
            <Text style={styles.here}>Upgrade here:</Text>
            <TouchableOpacity
              style={[
                styles.buyButton,
                productIdentifier === 'matter_premium_weekly_prod' &&
                  styles.current,
              ]}
              activeOpacity={1}
              onPress={() => {
                if (productIdentifier === 'matter_premium_weekly_prod') {
                } else {
                  setShowLoader(true);
              ...
              {productIdentifier === 'matter_premium_weekly_prod' && (
                <View style={styles.tryNow}>
                  <Text style={styles.try}>Your current deal</Text>
                </View>
              )}
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.buyButton,
                productIdentifier === 'matter_premium_monthly_prod' &&
                  styles.current,
              ]}
              onPress={() => {
                if (productIdentifier === 'matter_premium_monthly_prod') {
                } else {
                  setShowLoader(true);
            ...
            <TouchableOpacity
              style={[
                styles.buyButton,
                productIdentifier === 'matter_premium_quarterly_prod' &&
                  styles.current,
              ]}
              onPress={() => {
                if (productIdentifier === 'matter_premium_quarterly_prod') {
                } else {
                  setShowLoader(true);
...
const styles = StyleSheet.create({
  ...
  current: {
    borderColor: AppColors.MainColor + '99',
    color: AppColors.AppBlack + '61',
  },
  priceText: {
